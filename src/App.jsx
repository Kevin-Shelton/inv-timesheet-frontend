...
function TaskBasedTimesheetPage() {
  ...
  const getHoursForDay = (date) => {
    ...
    const dayEntries = timesheetEntries.filter(entry => entry.date === dateStr)
    return dayEntries.reduce((total, entry) => total + (entry.hours || 0), 0)
  }

  const getTotalWeekHours = () => {
    return weekDates.reduce((total, date) => total + getHoursForDay(date), 0)
  }
  ...
  const handleManualEntrySubmit = async (e) => {
    ...
    try {
      const entryData = {
        ...
        hours: entryMode === 'hour' ? parseFloat(manualEntry.hours) : calculateHoursFromTime(),
        description: manualEntry.note || `${manualEntry.activity} - ${manualEntry.project}`,
        campaign_id: campaigns.find(c => c.name === manualEntry.activity)?.id
      }
  ...
  const calculateHoursFromTime = () => {
    ...
    const start = new Date(`2000-01-01T${manualEntry.startTime}`)
    const end = new Date(`2000-01-01T${manualEntry.endTime}`)
  ...
  return (
    <div className="jibble-timesheet-page">
      {/* Header */}
          ...
          <div className="empty-state">
            <div className="empty-state-icon">
              <div className="payroll-icon">
                ...
                <CheckCircle className="w-4 h-4 check-overlay" />
              </div>
            </div>
            ...
            <div className="sidebar-content">
              ...
              <div className="user-section">
                ...
                <div className="user-details">
                  ...
                  <p className="user-meta">Start time: 12:00 am</p>
                  <p className="user-meta">No previous entry</p>
                </div>
              </div>
              ...
              <form onSubmit={handleManualEntrySubmit} className="entry-form">
                {entryMode === 'time' ? (
                  <div className="time-entry-section">
                    ...
                    <div className="calculated-hours">
                      {calculateHoursFromTime().toFixed(2)} hours
                    </div>
                  </div>
                ) : (
                ...
                <div className="entry-actions">
                  ...
                  <button type="button" className="duplicate-btn">
